...
if($method == "GET"){
    ...
    if (!isset($_GET["id"]) && !isset($_GET["pk_clientes"])){
        ...
        // listar todos os registros para um cliente específico
        try {
           
            $sql = "SELECT favoritos.pk_id, fk_noticia, fk_clientes, clientes.nome, noticia.titulo, noticia.imagem
                    FROM favoritos
                    JOIN noticia ON favoritos.fk_noticia = noticia.pk_id
                    JOIN clientes ON favoritos.fk_clientes = clientes.pk_id
...
if($method == "POST"){
    ...
    $fk_noticia = trim($dados->pk_noticia); // acessa valor de um OBJETO
    $fk_clientes = trim($dados->pk_clientes); // acessa valor de um OBJETO
   
    try {
       
        if(empty($dados->pk_noticia)){
            // está vazio  : ERRO
            throw new ErrorException("Valor da fk_noticia é inválido", 1);
        }
        if(empty($dados->pk_clientes)){
            // está vazio  : ERRO
...
if($method == "DELETE"){
    try {
        ...
        $sql = "DELETE FROM favoritos WHERE pk_id = :id";
